...
const actions = {
  ...
  async getVehicleDetail({ commit, dispatch }, payload) {
    return await vehicleService
      .show(payload.id, payload.spot_id)
      .then((data) => {
        return data.values;
      })
      .catch((error) => {
  ...
  async actionVehicle({ commit, dispatch }, payload) {
    return await vehicleService
      .action(payload.spot_id, payload)
      .then((data) => {
        dispatch(
  ...
  async createVehicle({ commit, dispatch }, payload) {
    return await vehicleService
      .create(payload.spot_id, payload)
      .then((data) => {
        dispatch(
          "modules/utility/storeUtility/setDefaultSuccessAlert",
          {
  ...
  async updateVehicle({ commit, dispatch }, payload) {
    return await vehicleService
      .update(payload.id, payload.spot_id, payload)
      .then((data) => {
        dispatch(
          "modules/utility/storeUtility/setDefaultSuccessAlert",
          {
  ...
  async deleteVehicle({ commit, dispatch }, payload) {
    return await vehicleService
      .remoce(payload.id, payload.spot_id)
      .then((data) => {
        dispatch(
          "modules/utility/storeUtility/setDefaultSuccessAlert",
          {
