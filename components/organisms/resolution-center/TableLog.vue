<script>
import constants from "~/constants/resolution-center";
...
export default {
  ...
  data: () => ({
    ...
    LOG_STATUS: constants.LOG_STATUS,
  }),
  async mounted() {
...
<template>
  <div class="row">
    <square-card
      :has_shadow="false"
      :has_border="false"
      additional_class="pt-0"
    >
      <h5 class="font-weight-bolder" :id="`${log_name}__header`">
        Log {{ log_name }}
      </h5>
            ...
            <tbody>
              <tr v-for="(log, index) in logs" :key="index">
                <td class="text-center">
                  <overview-label
                    :ref="`${log_name}__row__${index}__id`"
                    :value="log.id"
                    limit_text="8"
                    :is_italic="false"
                  />
                ...
                <td
                  class="text-center"
                  :id="`${log_name}__row__${index}__type`"
                >
                  {{ LOG_STATUS(log.type) }}
                </td>
                <td class="text-left">
                  <overview-label
                    :ref="`${log_name}__row__${index}__message`"
                    :value="log.message"
                    limit_text="45"
                    :has_space="false"
                    :is_italic="false"
                ...
                <td class="text-center">
                  <plain-label
                    :value="log.officer_detail ? log.officer_detail.name : '-'"
