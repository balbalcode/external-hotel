    ...
    <div class="row">
      <template v-for="gate in gates">
        <div
          ...
          v-for="(url, index) in data[gate.value]"
          :key="`photo-${gate.value}-${index}`"
        >
          <h6 class="font-weight-bold text-center">
            {{ gate.text }} Kamera {{ index + 1 }}
          </h6>
          <div class="row mx-0 mb-2 bg-gray-80" style="height: 200px">
            <content-image
              :value="url"
              additional_class="h-100 object-contain align-middle mx-auto"
            />
          </div>
          <active-button
            @click="toggleModalPreview(url)"
            size="sm"
            additional_class="w-100 mb-3"
            text="Lihat ukuran penuh"
          />
        </div>
      </template>
    </div>
