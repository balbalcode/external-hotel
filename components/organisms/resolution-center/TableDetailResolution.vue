...
export default {
  components: {
    ...
    ParkingImageDetail: () => import("./ParkingImageDetail"),
  },
  props: {
    data: { default: "" },
  },
  ...
  computed: {
    image_list() {
      ...
      if (this.data.image_list) {
        if (
          this.data.image_list.image_url &&
          this.data.image_list.image_url.length
        )
          image_list.image_url = this.data.image_list.image_url;
        if (
          this.data.image_list.image_out_url &&
          this.data.image_list.image_out_url.length
        )
  ...
  <div>
    <div class="row">
      <div class="col-12">
        ...
        <label class="mr-3"
          >ID Transaksi : <strong>{{ data.id }}</strong></label
        >
        <label class="mr-3">
        ...
        <label class="mr-3">
          <span v-if="data.ref_id && data.ref_type === 'FINDER'">
            Plat Nomor:
            <strong>{{
              data.ref_detail.values.motorcycle_detail.license_plate
            }}</strong>
          </span>
          <span
            v-else
            ...
            >Plat belum tersedia! <i class="bx bx-error-circle"></i
          ></span>
        </label>
          ...
          <table class="table table-striped table-borderless mb-0 custom-tbl">
            <thead class="thead-freeze bg-gray-70">
              <tr>
                ...
                <th
                  v-if="data.ref_id && data.ref_type === 'FINDER'"
                  class="text-center"
                >
                  Finder
                </th>
                ...
                <td class="text-center">
                  <plain-label
                    :value="
                      options
                        .TRANSACTION_STATUS()
                        .filter((opt) => data.status === opt.value)[0].text
                    "
                  />
                </td>
                <td class="text-center">
    ...
    <div
      class="row my-3"
      v-if="
        data.prediction.lpr_in.license_plate &&
        data.prediction.lpr_out.license_plate
      "
          ...
          <table class="table table-striped table-borderless mb-0 custom-tbl">
            ...
            <tbody>
              <tr>
                ...
                <td class="text-center">
                  {{ data.prediction.lpr_in.processed_time }}
                </td>
                <td class="text-center">
                  {{ data.prediction.lpr_in.vehicle }}
                </td>
              </tr>
              <tr>
                ...
                <td class="text-center">
                  {{ data.prediction.lpr_out.processed_time }}
                </td>
                <td class="text-center">
                  {{ data.prediction.lpr_out.vehicle }}
                </td>
              </tr>
            </tbody>
          </table>
