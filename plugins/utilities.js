...
export default {
  ...
  fireToast(
    title = "berhasil",
    type = "success",
    content = "berhasil memperbarui data",
    opener = "",
    duration = 3000
  ) {
    ...
    window.$nuxt.$store.dispatch(
      "modules/utility/storeUtility/setAlert",
      payload
    );
    return payload;
  },
  ...
  async getSpotInfo(spot_id) {
    ...
    let spot = await window.$nuxt.$store.state.modules.spot.storeSpot
      .currentSpot;
    if (spot.id) {
      return spot;
    } else {
      return await window.$nuxt.$store.dispatch(
        "modules/spot/storeSpot/getSpotDetail",
        { id: spot_id }
      );
    }
  },
  ...
  async getSpotMembershipType(spot_id) {
    ...
    let spot = await window.$nuxt.$store.state.modules.spot.storeSpot
      .currentSpot;
    if (!spot.id)
      spot = await window.$nuxt.$store.dispatch(
        "modules/spot/storeSpot/getSpotDetail",
        { id: spot_id }
      );
    if (
      spot.additional_information &&
      spot.additional_information.membership_type &&
  ...
  dateUTCToLocal(date, format = "DD-MM-YYYY", timezone = "") {
    ...
    return this.formatLocalTimezone(
      utc_date.getTime(),
      format ?? "DD-MM-YYYY",
      timezone ?? ""
    );
  ...
  async getOptionProductMembership() {
    let data = await window.$nuxt.$store.state.modules.product.storeProduct
      .PRODUCT_LIST;
    ...
    if (data?.values?.length) {
      window.$nuxt.$store.commit(
        "modules/product/storeProduct/SET_PRODUCT",
        data
      );
  ...
  getMembershipStatus(start, end) {
    start =
      parseInt(moment_timezone(start).tz(this.getSpotTimezone()).unix()) * 1000;
    end =
      parseInt(moment_timezone(end).tz(this.getSpotTimezone()).unix()) * 1000;
    let today = new Date();
      ...
      for (var i = 1; i < lines.length; i++) {
        ...
        for (var j = 0; j < headers.length; j++) {
          if (j === headers.length - 1)
            current_line[j] = current_line[j].slice(0, -1);
  ...
  checkNullableData(data) {
    if (typeof data === "undefined") {
      window.$nuxt.$sentry.captureMessage(
        "unexpected data format in checkNullableData, params send undefined please check ASAP"
      );
      return true;
    }
    try {
  ...
  encodeReportFilterUrl(data = []) {
    ...
    try {
      ...
      this.fireToast(
        "Gagal",
        "error",
        "Gagal membuat request untuk mengirim report"
      );
      throw error;
    }
  },
  ...
  async getVehicle() {
    try {
      let vehicles = await window.$nuxt.$store.state.modules.vehicle
        .storeVehicle.VEHICLE_GROUP;
      if (!vehicles) {
        const spot_id = await this.getSpotId();
        vehicles = await window.$nuxt.$store.dispatch(
          "modules/vehicle/storeVehicle/getVehicle",
          { spot_id }
        );
      }
  ...
  async getFormattedVehicle() {
    try {
      let data = await window.$nuxt.$store.state.modules.vehicle.storeVehicle
        .VEHICLE;
      // checking is the product state has a row or not
      if (!data) {
        data = [];
        ...
        window.$nuxt.$store.dispatch(
          "modules/vehicle/storeVehicle/storeVehicle",
          data
        );
      }
      return data;
      ...
      window.$nuxt.$sentry.captureMessage(
        "Fault error utility getFormattedVehicle"
      );
      throw error;
    }
  },
  ...
  async getVehicleMap() {
    try {
      let map = await window.$nuxt.$store.state.modules.vehicle.storeVehicle
        .VEHICLE_MAP;
      if (!map) {
        map = {};
        ...
        window.$nuxt.$store.dispatch(
          "modules/vehicle/storeVehicle/mapVehicle",
          map
        );
      }
      return map;
  ...
  setErrorContextSentry(object, origin, name = "error-detail") {
    try {
      ...
      if (object.hasOwnProperty("response")) {
        sentry_message = {
          message: object.response.hasOwnProperty("data")
            ? object.response.data.message
            : `${object.response.statusText} - ${object.response.status}`,
      ...
      console.log(e);
      window.$nuxt.$sentry.captureMessage(
        "error function setErrorContextSentry in utilities, fix bugs ASAP"
      );
    }
  },

  processEliminateArrayList(
    source = [],
    selected_values = [],
    finder = "value"
  ) {
    try {
      ...
      window.$nuxt.$sentry.captureMessage(
        "error processEliminateArrayList in utilities"
      );
